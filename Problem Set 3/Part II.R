...
> library(lattice)
Warning message:
  package 'lattice' was built under R version 3.0.3 
> 
...
Error in eval(expr, envir, enclos) : could not find function "Length"
> ddply(gDat, ~continent, summarize, nUniqCountries = length(unique(country)))
...
> jFun <- function(x) {
  +     estCoefs <- coef(lm(lifeExp ~ I(year - yearMin), x))
  +     names(estCoefs) <- c("intecept", "slope")
  +     return(estCoefs)
  + }
...
Installing package into 'C:/Users/Trevor/Documents/R/win-library/3.0'
...
C:\Users\Trevor\AppData\Local\Temp\Rtmp8cl2b6\downloaded_packages
> library(xtable)
> 
  > set.seed(916)
...
<!-- html table generated in R 3.0.2 by xtable 1.7-3 package -->
  <!-- Sun Apr 20 14:46:31 2014 -->
  <TABLE border=1>
  <TR> <TH> country </TH> <TH> intecept </TH> <TH> slope </TH>  </TR>
...
$ intecept : num  29.9 59.2 43.4 32.1 62.7 ...
$ slope    : num  0.275 0.335 0.569 0.209 0.232 ...
> tail(jCoefs)
...
Error in order(intercept) : object 'intercept' not found
> foo <- arrange(foo, intercept)
Error in order(intercept) : object 'intercept' not found
> foo <- arrange(foo, intecept)
> ##foo <- foo[order(foo$intercept), ] # an uglier non-plyr way
  > foo <- xtable(foo)
> print(foo, type = "html", include.rownames = FALSE)
<!-- html table generated in R 3.0.2 by xtable 1.7-3 package -->
  <!-- Sun Apr 20 14:57:34 2014 -->
  <TABLE border=1>
  <TR> <TH> country </TH> <TH> continent </TH> <TH> intecept </TH> <TH> slope </TH>  </TR>
